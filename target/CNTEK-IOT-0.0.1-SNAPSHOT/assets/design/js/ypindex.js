...
$(function(){
	...
	$('.i_attrName').next().css({width:'185px'});
	deviced.bg={
		x:0,
		y:0,
	...
	$('#i_scroll').change(function(){
		...
		$('#resizeBox').css({overflow:deviced.bg.scroll});
	});
	...
	$('#controlNameBox > li').draggable({
   	...
   	function createDev($target,left,top,html,DevFlag){
		if(falg){
			...
			falg=false;
			$target.append(html);
			$('#'+devId).css({position:'absolute',left:left,top:top});
			...
			if(deviced[devId].devIndex==8){
				...
				$('.ctrlb').css({border:'2px solid #ccc',background:'transparent',width:'20px',height:'20px',lineHeight:'20px',margin:'3px',color:'#fff',fontSize:'16px',fontWeight:'bold'}).hover(function(){
					$(this).css({backgroundColor:'rgba(239, 129, 33, 0.8)'});
				},function(){
					$(this).css({background:'transparent'});
				});
				$('.yt2').css({marginLeft:'56px',marginRight:'53px',background:'transparent url(./images/up.png)'});
				$('.yt4').css({background:'transparent url(./images/left.png)',marginLeft:'6px',});
				$('.yt5').css({background:'transparent url(./images/auto.png)',marginLeft:'3px',marginRight:'3px'});
				$('.yt6').css({background:'transparent url(./images/right.png)',marginRight:'3px'});
				$('.yt8').css({marginLeft:'56px',marginRight:'53px',background:'transparent url(./images/down.png)'});
				$('.yt').css({display:'inline-block',width:'47px',height:'47px',borderRadius:'50%',border:'3px solid #ccc'}).hover(function(){
					$(this).css({position:'relative',left:'-1px',top:'-2px',backgroundColor:'rgba(239, 129, 33, 0.8)'});
				},function(){
					$(this).css({position:'relative',left:0,top:0,backgroundColor:'transparent'});
				});
				$('.prePoint').css({color:'#fff',width:'80px',height:'30px',background:'rgba(0,0,0,0.4)',borderRadius:'5px',marginLeft:'5px'}).hover(function(){
					$(this).css({backgroundColor:'rgba(239, 129, 33, 0.8)'});
				},function(){
				...
				onStopDrag:function(e){
				    ...
				    if(deviced[zoneId].devFlag=='groupBox'){
				    	...
				    	if(zoneBoxHeight<(devLeft+2)||zoneBoxWidth<(devTop+2)||devLeft<devWidth||devTop<devHeight){
				    		...
				    		$(this).css({left:(zoneLeft+devLeft)+'px',top:(zoneTop+devTop)+'px'});
				    	} 	
				    }
	...
	$('#i_w').blur(function(){
		...
		if($('#resizeBox').css('outline-style')!=='none'){
			deviced.bg.w=w;
			$('#resizeBox').css({width:w+'px'});
		}
		var devArr= $('.dev');
		for(var i=0;i<devArr.length;i++){
			if(devArr.eq(i).css('outline-style')!=='none'){
				devArr.eq(i).css({width:(w+'px')});
				deviced[devArr.eq(i).attr('id')].w=w;

				if(deviced[devArr.eq(i).attr('id')].devIndex==14){
					$('#i_h').val(w);
					devArr.eq(i).css({height:(w+'px')});

					var resId = devArr.eq(i).attr('id');
					$('#'+resId+'img').css({width:'100%',height:'100%'});
	...
	$('#i_h').blur(function(){
		...
		if($('#resizeBox').css('outline-style')!=='none'){
			deviced.bg.h=h;
			$('#resizeBox').css({height:h+'px'});
		}
		var devArr= $('.dev');
		for(var i=0;i<devArr.length;i++){
			if(devArr.eq(i).css('outline-style')!=='none'){
				devArr.eq(i).css({height:(h+'px')});
				deviced[devArr.eq(i).attr('id')].h=h;
				if(deviced[devArr.eq(i).attr('id')].devIndex==14){
					$('#i_w').val(h);
					devArr.eq(i).css({width:(h+'px')});
					//随着外框的扩大，绘制区域也需放大
					var resId = devArr.eq(i).attr('id');
					$('#'+resId+'img').css({width:'100%',height:'100%'});
	...
	$('#i_borderwidth').blur(function(){
		...
		if($('#resizeBox').css('outline-style')!=='none'){
			...
			$('#resizeBox').css({borderWidth:(borderWidth+'px')});
		}
		var devArr= $('.dev');
		for(var i=0;i<devArr.length;i++){
			if(devArr.eq(i).css('outline-style')!=='none'){
				devArr.eq(i).css({borderWidth:(borderWidth+'px')});
	...
	$("#i_bordercolor").on('hide.spectrum', function(e, tinycolor) {
		...
		if($('#resizeBox').css('outline-style')!=='none'){
			...
			$('#resizeBox').css({borderColor:color});
		}
		var devArr =$('.dev');
		for(var i=0;i<devArr.length;i++){
			if(devArr.eq(i).css('outline-style')!=='none'){
				devArr.eq(i).css({borderColor:color});
				devArr.eq(i).attr('data-bfalg','true');
	...
	$('#i_padding').blur(function(){
		...
		for(var i=0;i<devArr.length;i++){
			if(devArr.eq(i).css('outline-style')!=='none'){
				devArr.eq(i).css({padding:padd+'px'});
	...
	$('#i_inverse').change(function(){
		...
		for(var i=0;i<devArr.length;i++){
			if(devArr.eq(i).css('outline-style')!=='none'){
				...
				$('#'+nowId+'img').css({backgroundImage:"url("+deviced[nowId].openImgurl+")"});
			}
		}	
	});
	//仪表盘告警上下限设置
	...
	$('#i_bjcolor').on('hide.spectrum', function(e, tinycolor) { 
		...
		if($('#resizeBox').css('outline-style')!=='none'){
			...
			$('#resizeBox').css({backgroundColor:normalColor});
			return false;
		}
		var devArr= $('.dev');
		for(var i=0;i<devArr.length;i++){
			if(devArr.eq(i).css('outline-style')!=='none'){
				...
				if(deviced[devArr.eq(i).attr('id')].devIndex==8||deviced[devArr.eq(i).attr('id')].devIndex==14||deviced[devArr.eq(i).attr('id')].devIndex==23){
					devArr.eq(i).css({background:normalColor});
					...
					switch(deviced[devArr.eq(i).attr('id')].MapType){
						case 'BMap':
							deviced[devArr.eq(i).attr('id')].marker[markerID].Label.setStyle({background:normalColor});
							break;
						case 'AMap':
							break;
						case 'qq':
					...
					$('#'+upId).css({backgroundColor:normalColor});
				}
	...
	$('#i_tagtype').change(function(){
		...
		for(var i=0;i<devAttr.length;i++){
			if(devAttr.eq(i).css('outline-style')!=='none'){
				...
				devAttr.eq(i).css({width:'100px',height:'30px'});
				deviced[tagId].w = '100';
				deviced[tagId].h = '30';
				...
				if(tagtype=='pic'){
					...
					$('#'+tagId+'img').css({background:"url("+deviced[tagId].bgImg+") no-repeat"}).text('');
					...
					$('#'+tagId+'img').text('').css({background:'none',backgroundColor:deviced[tagId].bgColor});
					...
					$('#'+tagId+'img').text('').css({background:"url("+deviced[tagId].bgImg+") no-repeat"});
	...
	$('#i_showmode').change(function(){
		...
		if($('#resizeBox').css('outline-style')!=='none'){
			...
			switch(showmode){
				case 'normal':
					$('#resizeBox').css({background:"url("+deviced.bg.bgImg+") no-repeat",backgroundSize:'contain',backgroundColor:deviced.bg.bgColor});
					break;
				case 'strech':
					$('#resizeBox').css({background:"url("+deviced.bg.bgImg+") no-repeat",backgroundSize:'cover'});
					break;
				case 'repeat':;
					$('#resizeBox').css({background:"url("+deviced.bg.bgImg+") repeat"});
					break;
				case 'none':
					$('#resizeBox').css({background:"none",backgroundColor:deviced.bg.bgColor});
					break;
			}
		...
		for(var i=0;i<devAttr.length;i++){
			if(devAttr.eq(i).css('outline-style')!=='none'){
				...
				switch(showmode){
					case 'normal':
						$('#'+upDataId+'img').css({background:"url("+deviced[upDataId].bgImg+") no-repeat",backgroundSize:'contain',backgroundColor:deviced[upDataId].bgColor});
						break;
					case 'strech':
						$('#'+upDataId+'img').css({background:"url("+deviced[upDataId].bgImg+") no-repeat",backgroundSize:'cover'});
						break;
					case 'repeat':;
						$('#'+upDataId+'img').css({background:"url("+deviced[upDataId].bgImg+") repeat"});
						break;
					case 'none':
						$('#'+upDataId+'img').css({background:"none",backgroundColor:deviced[upDataId].bgColor});
						break;
				}
			}
		}
	...
	$('#i_btnType').change(function(){
		...
		for(var i=0;i<devAttr.length;i++){
			if(devAttr.eq(i).css('outline-style')!=='none'){
				...
				devAttr.eq(i).css({width:'100px',height:'60px',borderRadius:'0'});
				$('#'+btnId+'img').css({width:'100%',height:'100%',borderRadius:'10%'});
				if(btnType=='general'){
					deviced[btnId].openImgurl="./images/11.png";
					deviced[btnId].closeImgurl="./images/c11.png";
					devAttr.eq(i).find('button').css({backgroundImage:"url("+deviced[btnId].openImgurl+")",backgroundColor:deviced[btnId].bgColor,color:deviced[btnId].fontColor}).text('');
				}else if(btnType=='ctrl'){
					deviced[btnId].bgImg = "./images/btn.png";
					deviced[btnId].sendValue = '0';
					devAttr.eq(i).find('button').css({backgroundImage:"url("+deviced[btnId].bgImg+")",backgroundColor:deviced[btnId].bgColor,color:deviced[btnId].fontColor}).text('');
		...
		for(var i=0;i<devAttr.length;i++){
			if(devAttr.eq(i).css('outline-style')!=='none'){
				...
				if(deviced[btnId].btnType=='general'){
					...
					devAttr.eq(i).find('button').css({backgroundImage:"url("+deviced[btnId].openImgurl+")",backgroundColor:deviced[btnId].bgColor,color:deviced[btnId].fontColor}).text(deviced[btnId].openText);
					...
					devAttr.eq(i).find('button').css({backgroundImage:"url("+deviced[btnId].bgImg+")",backgroundColor:deviced[btnId].bgColor,color:deviced[btnId].fontColor}).text(deviced[btnId].tagtext);
				...
				if(btnStyle=="text"){
					if(deviced[btnId].shape=='rectangle'){
						devAttr.eq(i).css({width:'100px',height:'30px',borderRadius:'0'});
						$('#'+btnId+'img').css({width:'100%',height:'100%',borderRadius:'10%'});

					}else if(deviced[btnId].shape=='circle'){
						devAttr.eq(i).css({width:'50px',height:'50px',borderRadius:'50%'});
						$('#'+btnId+'img').css({width:'100%',height:'100%',borderRadius:'50%'});
					}
	...
	$('#i_tagtext').blur(function(){
		...
		for(var i=0;i<devArr.length;i++){
			if(devArr.eq(i).css('outline-style')!=='none'){
				...
				$('#p'+pId+'img').css({fontSize:deviced[pId].fontSize,color:deviced[pId].fontColor,fontWeight:deviced[pId].fontWeight,fontFamily:deviced[pId].fontFamily});
		...
		for(var i=0;i<devAttr.length;i++){
			if(devAttr.eq(i).css('outline-style')!=='none'){
				...
				if(deviced[fontId].devIndex=='15'){
					...
					$('#'+imgId).css({fontFamily:fontFamily});
				}
			}
		}
	});
		...
		for(var i=0;i<devAttr.length;i++){
			if(devAttr.eq(i).css('outline-style')!=='none'){
				...
				if(deviced[fontId].devIndex=='15'){
					...
					$('#'+imgId).css({fontWeight:fontWeight});
				}
			}
		}
	});
		...
		for(var i=0;i<devAttr.length;i++){
			if(devAttr.eq(i).css('outline-style')!=='none'){
				...
				if(deviced[fontId].devIndex=='15'){
					...
					$('#'+imgId).css({fontSize:fontSize+'px'});
				}
			}
		}
	});
						...
						if(groupArr.eq(j).css('outline-style')!=='none'){
							switch(str){
								...
								case 'pre':
									...
									groupArr.eq(j+1).css({zIndex:nowIndex});
									groupArr.eq(j).css({zIndex:num});
									deviced[groupArr.eq(j).attr('id')].zIndex = num;
		...
		for(var i=0;i<devAttr.length;i++){
			if(devAttr.eq(i).css('outline-style')!=='none'){
				...
				if(row<oldrow){
					...
					$('.smallocxdiv').css({width:width+'px',height:height+'px'});
				}else if(row>oldrow){
...
function cptzd(a,b,d,c){
	var RequestDataStart = {method:'ptz.start',session:'60286208&1489459650153',params:{channel:Number($('#channel').val()),code:b,arg1:d,arg2:0,arg3:0},id:1};
	var RequestDataStop = {method:'ptz.stop',session:'60286208&1489459650153',params:{channel:Number($('#channel').val()),code:b,arg1:d,arg2:0,arg3:0},id:1};
	var sendData='';
	if(a){
	...
	$('.yt').css({backgroundColor:'transparent'});
}
...
function CircleResizable(devId){
	$('#'+devId).resizable({  
        ...
        onStopResize : function (e) {  
            ...
            $('#'+devId).css({width:result,height:result,borderRadius:"50%"});
          
        	var updateId = devId+"img";
        	$('#'+updateId).css({width:result,height:result});
	...
	$('#'+devId).resizable({  
        ...
        onStopResize : function (e) {  
            ...
            if(deviced[devId].devIndex==23){
				...
				$('.smallocxdiv').css({width:hkWidth+'px',height:hkHeight+'px'});
            	return false;
	...
	if(deviced[changeId].shape=='rectangle'){
		if(deviced[changeId].devIndex==11){
			if(deviced[changeId].btnStyle=='pic'){
				$('#'+changeId).css({width:'100px',height:'60px',borderRadius:'0'});
			}else if(deviced[changeId].btnStyle=='text'){
				$('#'+changeId).css({width:'100px',height:'30px',borderRadius:'0'});
			}
		}else if(deviced[changeId].devIndex==8){
			if(deviced[changeId].devtype=='Definde'){
				$('#'+changeId).css({width:'100px',height:'130px',borderRadius:'0'});
			}
		}
		
		$('#'+changeId+'img').css({width:'100%',height:'100%',borderRadius:'0'});
	}else if(deviced[changeId].shape=='circle'){
		if(deviced[changeId].devIndex==11){
			$('#'+changeId).css({width:'50px',height:'50px',borderRadius:'50%',overflow:'hidden'});
		}else if(deviced[changeId].devIndex==8){
			$('#'+changeId).css({width:'100px',height:'100px',borderRadius:'50%'});
		}
		$('#'+changeId+'img').css({width:'100%',height:'100%',borderRadius:'50%'});
	}
...
function selectDevState(stateName,url){
	...
	$('#MonitoringSiteTable').datagrid({
		...
		columns: [[
			{field:'deviceid',title:'设备ID号',width:60,align:'center',hidden:'true'},
			{field:'orgid',title:'父ID号',width:60,align:'center',hidden:'true'},
			{field:'orgname',title:'设备组',width:70,align:'center'},
			{field:'devname',title:'设备名',width:70,align:'center'},
			{field:'Address',title:'模块',width:30,align:'center'},
			{field:'IndexOfType',title:'索引',width:30,align:'center'},
			{field:'Description',title:'值类型',width:70,align:'center'},
			{field:'CurValue',title:'当前值',width:30,align:'center'},
			{field:'RecvTime',title:'更新时间',width:80,align:'center'},
		]]
								...
								$('#i_chartcolor').on('hide.spectrum', function(e, tinycolor) {
									...
									for(var i=0;i<devArr.length;i++){
										if(devArr.eq(i).css('outline-style')!=='none'){
											...
											$('.triangle-down'+devId).eq(deviced[devId].moniIndex).css({borderTopColor:color});
	...
	var infoHtml = "<div id='openDoor'><div id='closebtn'style='position:absolute;right:10px;top:0px;width:10px;height:10px;font:bold 20px 微软雅黑;cursor:pointer;'>×</div><div id='avatar'style='width:150px;height:180px;margin-top:10px;margin-left:170px;border:1px solid #ccc;background:url(./images/head.jpg)'></div><table style='margin-left:100px;margin-top:5px;'><tr><td align='right'>部门：</td><td><input type='text'id='dep'></td></tr><tr><td align='right'>职位：</td><td><input type='text'id='pos'></td></tr><tr><td align='right'>姓名：</td><td><input type='text'id='Info'></td></tr><tr><td align='right'>卡号：</td><td><input type='text'id='CardWord'></td></tr><tr><td align='right'>开门控制点：</td><td><input type='text'id='OpenCtrl'></td></tr><tr><td colspan='2'><button id='openDoorOk'style='width:100px;margin:2px 5px 2px 15px;'>开门</button><button id='dopenDoorOk'style='width:100px;margin:2px 5px;'>确定</button></td></tr></table></div>";
		...
		if(parentId=='previewBox'){
			$('#openDoorOk').click(function(){
				var sendData = {openValue:data.openValue};
				$.ajax({
					url:"./php/random.php",
					data:sendData,
					type:'POST',
    	...
    	if (xhr.readyState == 4) {            
			if (xhr.status == 200) {               
				...
				if($('#resizeBox').css('outline-style')!=='none'){
					...
					$('#resizeBox').css({background:"url("+deviced.bg.bgImg+") no-repeat",backgroundSize:'contain'});
				}
				var devArr = $('.dev');
				for(var i=0;i<devArr.length;i++){
					if(devArr.eq(i).css('outline-style')!=='none'){
...
function chgDevtype(devId){
	switch(deviced[devId].devtype){
		case 'Switch':
			$('#'+devId).css({width:'100px',height:'130px',borderRadius:'0'});
			$('#'+devId+'img').css({borderRadius:'0'});
			deviced[devId].shape="rectangle";
			break;
		case 'Smoke':
			$('#'+devId).css({width:'100px',height:'100px',borderRadius:'50%'});
			$('#'+devId+'img').css({borderRadius:'50%'});
			deviced[devId].shape = 'circle';
			break;
		case 'Infrared':
			$('#'+devId).css({width:'100px',height:'130px',borderRadius:'0'});
		...
		case 'Definde':
			...
			break;
		case 'DefText':
			$('#'+devId).css({width:'100px',height:'30px',borderRadius:'0',overflow:'hidden',color:'#333'});
			$('#'+devId+'img').css({borderRadius:'0'});
			deviced[devId].shape='rectangle';
			break;
	}
...
function chg_i_devtype(devId){
	...
	textAlign(devId);
	$('#'+devId).trigger('click');
	$('#'+imgId).css({backgroundImage:"url("+deviced[devId].openImgurl+")"});
}
...
function textAlign(devId){
	...
	switch(deviced[devId].textAlign){
		case '0'://左上
			upElement.css({textAlign:'left',paddingTop:'0'});
			break;
		case '1'://中上
			upElement.css({textAlign:'center',paddingTop:'0'});
			break;
		case '2'://右上
			upElement.css({textAlign:'right',paddingTop:'0'});
			break;
		case '3':
			var range = parseInt((deviced[devId].h-upElement.height())*0.5);
			upElement.css({textAlign:'left',paddingTop:range+'px'});
			break;
		case '4':
			var range = parseInt((deviced[devId].h-upElement.height())*0.5);
			upElement.css({textAlign:'center',paddingTop:range+'px'});
			break;
		case '5':
			var range = parseInt((deviced[devId].h-upElement.height())*0.5);
			upElement.css({textAlign:'right',paddingTop:range+'px'});
			break;
		case '6':
...
function PrePicShowMode(devId){
	...
	switch(showmode){
		case 'normal':
			$('#pre'+devId).css({background:"url("+deviced[devId].bgImg+") no-repeat",backgroundSize:'contain'});
			break;
		case 'strech':
			$('#pre'+devId).css({background:"url("+deviced[devId].bgImg+") no-repeat",backgroundSize:'cover'});
			break;
		case 'repeat':;
			$('#pre'+devId).css({background:"url("+deviced[devId].bgImg+") repeat"});
			break;
		case 'none':
			$('#pre'+devId).css({background:"none"});
			break;
	}
}
	...
	deviced[devId].map.addEventListener("tilesloaded", function(e){   //获取地图加载完成后的中心点位置信息,及缩放等级
		...
		if(markerID!==null){
			...
			for(var i in deviced[devId].marker){
				...
				var Mpoint = new BMap.Point(pointLng,pointLat);
				var Label = new BMap.Label(label,{offset:new BMap.Size(20,-10)});//创建标注文本
		  		...
		  		Label.setStyle({background:labelBg});
...
function addMarker(devId,point,label,markerCount){
	...
	deviced[devId].marker[markerCount].Label = new BMap.Label(label,{offset:new BMap.Size(20,-10)});//创建标注文本
		...
		for(var i in deviced[devId].marker){
		    ...
		    // 添加label
		   	deviced[devId].marker[i].Label = new qq.maps.Label({
		        ...
		        map:deviced[devId].map,
		        content:label,
		        style:{background:deviced[devId].marker[i].bgColor}
		    });
		    	//标记点右键添加菜单，删除该标记点
...
function hkvisiionPlayZoom(devId){
   ...
   /* $('#playbtn').append('<button id="capture"data-dev="'+devId+'"class="ctrlbtn">抓图</button><button id="fullScreen"data-dev="'+devId+'"class="ctrlbtn">全屏</button>');
	...
	$('.smallocxdiv').css({width:smallocxdivWidth+'px',height:smallocxdivHeight+'px'});
...
function preview(){
	...
	if($.isEmptyObject(deviced)){
		...
		for(var i in deviced){
			if(deviced[i].devIndex=='bg'){
				...
				var preHtml = '<div style="width:'+deviced[i].w+'px;height:'+deviced[i].h+'px;border:'+deviced[i].borderWidth+'px solid '+deviced[i].borderColor+';position:absolute;left:'+(deviced[i].x)+'px;top:'+(deviced[i].y)+'px;padding:'+deviced[i].padding+'px;z-index:'+deviced[i].zIndex+';"><div id="pre'+i+'"style="width:100%;height:100%;font-size:'+deviced[i].fontSize+'px;font-weight:'+deviced[i].fontWeight+';font-family:'+deviced[i].fontFamily+';background-color:'+deviced[i].bgColor+';background-image:url('+deviced[i].bgImg+');color:'+deviced[i].fontColor+';"><p id="'+i+'txt">'+oText+'</p></div></div>';

			}else if(deviced[i].devIndex==5){

				var preHtml = '<div style="width:'+deviced[i].w+'px;height:'+deviced[i].h+'px;border:'+deviced[i].borderWidth+'px solid '+deviced[i].borderColor+';position:absolute;left:'+(deviced[i].x)+'px;top:'+(deviced[i].y)+'px;padding:'+deviced[i].padding+'px;z-index:'+deviced[i].zIndex+';"><div id="pre'+i+'"style="width:100%;height:100%;font-size:'+deviced[i].fontSize+'px;font-weight:'+deviced[i].fontWeight+';font-family:'+deviced[i].fontFamily+';color:'+deviced[i].fontColor+';background-color:'+deviced[i].bgColor+';"><p id="'+i+'txt"><span id="pre'+i+'name"></span><span id="pre'+i+'val"></span></p></div></div>';
			}else if(deviced[i].devIndex==15){
				...
				var preHtml = '<div style="width:'+deviced[i].w+'px;height:'+deviced[i].h+'px;border:'+deviced[i].borderWidth+'px solid '+deviced[i].borderColor+';position:absolute;left:'+(deviced[i].x)+'px;top:'+(deviced[i].y)+'px;padding:'+deviced[i].padding+'px;z-index:'+deviced[i].zIndex+';"><div id="pre'+i+'"style="width:100%;height:100%;font-size:'+deviced[i].fontSize+'px;font-weight:'+deviced[i].fontWeight+';font-family:'+deviced[i].fontFamily+';position:relative;background-color:'+deviced[i].bgColor+';background-image:url('+deviced[i].bgImg+');background-repeat:no-repeat;background-size:contain;"></div></div>';
				...
				if(deviced[i].chartType=='line'){
					for(var mi in deviced[i].series){
						...
						for(var num=0;num<=deviced[i].chartSplit;num++){
							chartSplitData.push({value:'',label:{normal:{show:false,position:'top'}}});
						}
						deviced[i].seriesData[mi]=chartSplitData;
						deviced[i].data.push({data:deviced[i].seriesData[mi]});
					}
				}else if(deviced[i].chartType=='bar'){
					for(var mi in deviced[i].series){
						var chartSplitData = [];
						chartSplitData.push({value:'',label:{normal:{show:true,position:'top'}}});
						deviced[i].seriesData[mi]=chartSplitData;
						deviced[i].data.push({data:deviced[i].seriesData[mi]});
					}
				}
			...
			if(deviced[i].shape=='circle'){
				$('#pre'+i).parent().css({borderRadius:'50%'});
				$('#pre'+i).css({borderRadius:'50%'});
			}
			
			if(deviced[i].devIndex=='bg'){
				switch(deviced.bg.showmode){
					case 'normal':
						$('#prebg').css({background:"url("+deviced.bg.bgImg+") no-repeat",backgroundSize:'contain',backgroundColor:deviced.bg.bgColor});
						break;
					case 'strech':
						$('#prebg').css({background:"url("+deviced.bg.bgImg+") no-repeat",backgroundSize:'cover',backgroundColor:deviced.bg.bgColor});
						break;
					case 'repeat':;
						$('#prebg').css({background:"url("+deviced.bg.bgImg+") repeat",backgroundColor:deviced.bg.bgColor});
						break;
					case 'none':
						$('#prebg').css({background:"none",backgroundColor:deviced.bg.bgColor});
						break;
				}
			}else if(deviced[i].devIndex==8){
				PreTextAlign(i);
				...
				if(deviced[i].btnType=='general'){
					$('#pre'+i).css({backgroundImage:"url("+deviced[i].openImgurl+")"}).text(deviced[i].openText);
				}else{
					$('#pre'+i).css({backgroundImage:"url("+deviced[i].bgImg+")"}).text(deviced[i].tagtext);
				}
				$('#pre'+i).click(function(){
					...
					if(deviced[upId].btnType=='general'){
						...
						var sendD = {state:deviced[upId].state};
					}else if(deviced[upId].btnType=='ctrl'){
						var sendD = {state:deviced[upId].sendValue};
					}
											...
											if(data==1){
												if(deviced[upId].btnType=='general'){
													if(preImg[upId].stateValue){
														$('#pre'+upId).text(deviced[upId].closeText).css({backgroundColor:deviced[upId].albgColor,backgroundImage:"url("+preImg[upId].closeImgurl.src+")"});
													}else{
									
														$('#pre'+upId).text(deviced[upId].openText).css({backgroundColor:deviced[upId].bgColor,backgroundImage:"url("+preImg[upId].openImgurl.src+")"});
													}
												}else{
													$('#pre'+upId).text(deviced[upId].tagtext).css({backgroundColor:deviced[upId].bgColor,backgroundImage:'url('+deviced[upId].bgImg+')'});
				...
				switch(deviced[i].showmode){
					case 'normal':
						$('#pre'+i).css({background:"url("+deviced[i].bgImg+") no-repeat",backgroundSize:'contain'});
						break;
					case 'strech':
						$('#pre'+i).css({background:"url("+deviced[i].bgImg+") no-repeat",backgroundSize:'cover'});
						break;
					case 'repeat':;
						$('#pre'+i).css({background:"url("+deviced[i].bgImg+") repeat"});
						break;
					case 'none':
						$('#pre'+i).css({background:"none",backgroundColor:deviced[i].bgColor});
						break;
				}
			}else if(deviced[i].devIndex==16){
				$('#pre'+i).click(function(){
	...
	for(var i in deviced){
		if(deviced[i].devIndex==22){
			...
			var jsonStr ={dev:i,devIndex:deviced[i].devIndex,devname:deviced[i].devname,marker:marker};
		}else{
			var jsonStr ={dev:i,devIndex:deviced[i].devIndex,devname:deviced[i].devname}
		}
							...
							if(data[i].devIndex==8){//判断该控件的类型，开关图片
								...
								if(data[i].nowValue==deviced[j].alarmValue){
									...
									if(data[i].nowValue==deviced[j].dopenvalue){
										$('#pre'+j+'').css({backgroundImage:"url("+img[j].openImg.src+")"});
										$('#'+j+'txt').text(openText);

									}else if(data[i].nowValue == deviced[j].dclosevalue){
										$('#pre'+j+'').css({backgroundImage:"url("+img[j].closeImg.src+")"});
										$('#'+j+'txt').text(closeText);

									}
									$('#pre'+j+'img').parent().css({backgroundColor:deviced[j].albgColor});
									...
									$('#pre'+j+'img').parent().css({backgroundColor:deviced[j].bgColor});
								}
							}else if(data[i].devIndex==14){//表盘

								if(data[i].nowValue<deviced[j].alarmMinValue||data[i].nowValue>deviced[j].alarmMaxValue){
									$('#pre'+j+'').parent().css({backgroundColor:deviced[j].albgColor});
								}else{
									$('#pre'+j+'').parent().css({backgroundColor:deviced[j].bgColor});
								}
								...
								if(data[i].nowValue<deviced[j].alarmMinValue||data[i].nowValue>deviced[j].alarmMaxValue){
									$('#pre'+j+'').css({backgroundColor:deviced[j].albgColor});
									if(paintalarm){
										paintalarm=false;
										paintnormal=true;
									...
									$('#pre'+j+'').css({backgroundColor:deviced[j].bgColor});
								...
								if(doorCount==0){
									...
									if(data[i].nowValue==deviced[j].openValue){
										$('#pre'+j).css({backgroundImage:"url("+deviced[j].closeImgurl+")"});
										...
										$('#pre'+j).css({backgroundImage:"url("+deviced[j].openImgurl+")"});
									}
								}
								...
								if((data[i].nowValue>deviced[j].alarmMaxValue)||(data[i].nowValue<deviced[j].alarmMinValue)){
									if(deviced[j].moniShowMode=='0'){
										...
										$('#pre'+j+'val').text(data[i].nowValue).css({backgroundColor:deviced[j].albgColor});
									}
								}else{
									if(deviced[j].moniShowMode=='0'){
										$('#pre'+j+'val').text('');
									}else{
										$('#pre'+j+'val').text(data[i].nowValue).css({backgroundColor:deviced[j].bgColor});
									}
								}
